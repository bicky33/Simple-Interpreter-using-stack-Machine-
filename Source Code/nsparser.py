...
class Parser:
    def __init__(self, tokens):
        self.tokens = tokens #set token setelah lexer
        self.pos = 0 #posisi token saat ini 
        self.poliz = [] #reverse
        self.buffer = [] #stack 
        self.addrsForFilling  = [] #menyimpan alamat polis
        self.addrsJumps  = [] #menyimpat alamat (array) untuk lompatan
        self.calls = [] 
    ...
    def if_stmt(self, pos):
        if (not self.KW_IF(self.pos)):
            ...
            self.parseExeption("ekspresi logika", self.tokens[self.pos][0])
            return False
        elif(not self.bkt_close(self.pos)):
            self.parseExeption(")", self.tokens[self.pos][0])
            return False
        ...
        if(not self.brace_close(self.pos)):
            self.parseExeption("}", self.tokens[self.pos][0])
            return False
        #else statement dan tidak akan mempengaruhi if statement
    ...
    def printing(self, pos):
        if (not self.KW_PRINT(self.pos)):
            ...
            self.parseExeption("string or ekspresi aritmatika", self.tokens[self.pos][0])
            return False
        elif (not self.bkt_close(self.pos)):
            self.parseExeption(")", self.tokens[self.pos][0])
            return False
    ...
    #fungsi inceremen dan decrement
    ...
    # Jika token input adalah angka, variabel, atau seleksi bersyarat
    #tambahkan ke stack umum atau stack cadangan 
    #urutkan berdasarkan token prioritas 
        ...
        ["INT", "FLOAT", "BOOL", "ID", "STRING"]):
            ...
            if (el[0] == ")"):#push to buffer
                ...
                if (lastCall == "WHILE"):
                    ...
                    # mengeksekusi hanya jika ada IF tanpa ELSE dan untuk percabangan normal,
                    # jika ada else 
                    self.poliz.append( (None, None) )           
            elif (el[0] != "(" and el[0] != "{" and len(self.buffer) != 0):
                
                # Jika token baru memiliki prioritas lebih rendah daripada yang terakhir di buffer, maka
                # buffer terakhir ditambahkan ke tumpukan utama, dan yang baru ditambahkan ke buffer  
                if (el[2] < self.endEl(self.buffer)[2]):                 
                    if (el[1] == "SEMICOLON"):
                        
                # jika akhir ekspresi terpenuhi, maka kirim semuanya dari buffer ke tumpukan utama

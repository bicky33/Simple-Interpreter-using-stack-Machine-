...
# Stack machine yang mengeksekusi kode itu sendiri
class StackMachine:
    def __init__(self, poliz):
        self.poliz = poliz #record dari poliz
        self.stack = [] #tumpukan utama untuk melakukan operasi
        self.variables = {} # Grammar variabel: varID -> varValue
        self.pos = 0 #menghitung jumlah poliz saat ini nantinya
    ...
    #lakukan perhitungan mesin utama
    def process(self):
    ...
    def inputting(self, var):
        if (self.variables.get(var) != None):
            ...
            print("Error: variable '" + var + "' tidak terdefinisi")
            exit()
    ...
    def convertType(self, value):
        try:
            ...
            print("Error: tipe data tidak diketahui '" + value + "'")
            exit()

    def checkDef(self, var):
        if (type(var) is tuple):
            if (self.variables.get(var[0]) == None and var[1] == "ID"):
                print("Error: variable '" + var[0] + "' tidak terdefinisi")
                exit()
    ...
    def calculate(self):
        ...
        #periksa apakah operator asignment atau 
        #variabel yang akan ditimpa atau diinisialisasian
        if (op == "="):
            a = a[0]
            #variabel atau angka 
            ...
            #optimasi apakah ada variabel yang tidak digunakan
            self.checkDef(a)
            self.checkDef(b)
    ...
    def mod(self, num1, num2):
        if (type(num1) == float or type(num2) == float):
            print("Error: modulus dari floar ")
            exit()
        elif num2 == 0:
            print("Error: modulus oleh 0 -_- ")
            exit()
        return num1 % num2, "INT"
    ...
    def modAssign(self, var, num):
        if (type(self.variables.get(var)) != float and type(num) != float):
            print("Error: modulus oleh float -_- ")
            exit()
        elif num == 0:
            print("Error: modulus oleh 0")
            exit()
        else:
            self.variables[var] = self.variables.get(var) % num
    ...
    def l_not(self, num):
        if (type(num) == bool):
            ...
            self.pushError("Error: menggunakan LOGICAL OR untuk ekspresi non-logis ")
    ...
    def compareException(self, n1, n2):
        self.pushError("Error: mustahil untuk membandingkan '" +
        str(n1) + "' dan '" + str(n2) + "' values")
